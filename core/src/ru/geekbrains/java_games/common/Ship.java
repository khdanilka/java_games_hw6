...
//        if(damageAnimationTimer >= DAMAGE_ANIMATION_INTERVAL) frame = 0;
    ...
    public boolean isKilled(Object object){

        ArrayList<Bullet> ar = bulletPool.getActiveObjects();
        for(Bullet a: ar){
            if ((a.getOwner() instanceof MainShip)&&(object instanceof MainShip)){
                continue;
            }
            if ((a.getOwner() instanceof Enemy)&&(object instanceof Enemy)){
                continue;
            }
            if (isBulletCollision(a)) {
                return true;
            }
        }
        return false;
